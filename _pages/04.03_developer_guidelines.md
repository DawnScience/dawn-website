...
1. [**1 **Introduction](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Introduction)
    1. [**1.1 **1. Eclipse and Buckminster](http://www.dawnsci.org/documentation/developer-guidelines#TOC-1.-Eclipse-and-Buckminster)
        1. [**1.1.1 **2. Clean workspace](http://www.dawnsci.org/documentation/developer-guidelines#TOC-2.-Clean-workspace)
        2. [**1.1.2 **3. CQuery](http://www.dawnsci.org/documentation/developer-guidelines#TOC-3.-CQuery)
        3. [**1.1.3 **4. Compile and Run](http://www.dawnsci.org/documentation/developer-guidelines#TOC-4.-Compile-and-Run)

2. [**2 **Plotting](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Plotting)
    1. [**2.1 **Getting started with IPlottingSystem](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Creating-the-Plotting-UI)
        1. [**2.1.1 **A Minimum View to Plot Something in DAWN](http://www.dawnsci.org/documentation/developer-guidelines#TOC-A-Minimum-View-to-Plot-Something-in-DAWN)

    2. [**2.2 **Plotting Something](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Plotting-Something)
        1. [**2.2.1 **Thread Safe Convenience Methods](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Thread-Safe-Convenience-Methods)
        2. [**2.2.2 **ITrace API](http://www.dawnsci.org/documentation/developer-guidelines#TOC-ITrace-API)
        3. [**2.2.3 **Line Traces for 1D](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Line-Traces-for-1D)
        4. [**2.2.4 **Image Traces for 2D](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Image-Traces-for-2D)
        5. [**2.2.5 **Making Image Traces Update Fast](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Making-Image-Traces-Update-Fast)
        6. [**2.2.6 **Surface Traces for 2D/3D](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Surface-Traces-for-2D-3D)

    3. [**2.3 **Listeners](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Listeners)

3. [**3 **Services](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Services)
    1. [**3.1 **Loader Service / Loader Factory](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Loader-Service-Loader-Factory)
    2. [**3.2 **Adding a FileLoader](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Adding-a-FileLoader)
    3. [**3.3 **Thumbnail Service](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Thumbnail-Service)
    4. [**3.4 **Persistence Service](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Persistence-Service)

4. [**4 **Regions](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Regions)
5. [**5 **Tools](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Tools)
    1. [**5.1 **Tool Pages](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Tool-Pages)
    2. [**5.2 **Tool Actions](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Tool-Actions)
    3. [**5.3 **Tools and Perspectives](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Tools-and-Perspectives)

6. [**6 **Slicing](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Slicing)
    1. [**6.1 **Conclusion](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Conclusion)

7. [**7 **HDF5 Libraries](http://www.dawnsci.org/documentation/developer-guidelines#TOC-HDF5-Libraries)
    1. [**7.1 **Simple HDF5 Reading](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Simple-HDF5-Reading)
    2. [**7.2 **Using the Factory (Expert use only)](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Using-the-Factory-Expert-use-only-)
    3. [**7.3 **Using the Low Level API (God use only)](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Using-the-Low-Level-API-God-use-only-)

8. [**8 **Connecting Plot to Python](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Connecting-Plot-to-Python)
9. [**9 **Unit/Regression Testing](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Unit-Regression-Testing)
10. [**10 **General Eclipse RCP stuff](http://www.dawnsci.org/documentation/developer-guidelines#TOC-General-Eclipse-RCP-stuff)
    1. [**10.1 **How to contribute to the DAWN welcome/intro page](http://www.dawnsci.org/documentation/developer-guidelines#TOC-How-to-contribute-to-the-DAWN-welcome-intro-page)
    2. [**10.2 **Vertical and horizontal scroll bars on DAWN views](http://www.dawnsci.org/documentation/developer-guidelines#TOC-Vertical-and-horizontal-scroll-bars-on-DAWN-views)
...
**public****class** AViewToPlotSomething
**extends** ViewPart {

       **private** IPlottingSystem plotting;

**       private** Logger logger = LoggerFactory._getLogger_(AViewToPlotSomething.**class**);

       **public** AViewToPlotSomething() {

             **try** {

                    **this**.plotting = PlottingFactory._createPlottingSystem_();
...
           plotting.createPlotPart(parent, "My Plot
Name",
getViewSite().getActionBars(), PlotType._IMAGE_, **this**);
...
       **public** Object getAdapter(**final**

Class clazz)
{

             **if** (IPlottingSystem.**class** == clazz) **return**plotting;

             **if** (IToolPageSystem.**class** == clazz) **return**plotting;
...
  system.createPlot1D(indices, Arrays.asList(

ls), monitor); // monitor may be null.

##### []()ITrace API

The
 low level or 'Trace' API allows all plots, in 1D, 2D or 3D to be dealt 
...
       imageTrace.setData(data,
**null**, **false**);

##### []()Surface Traces for 2D/3D

From
 Raptor onwards Dawn will begin to support more 3D operations in the 
plotting API. An example of this is the new ISurfaceTrace which is 
accessible using system.createSurfaceTrace(...).

#### []()Listeners

The
 system is event based - meaning that when almost anything changes in 
 ...
 ** Listener** ** Function**
 ...
  ITraceListener  Notification
 of when traces are about to be plotted, have been added, have been 
deleted etc. The TraceWillPlot event allows the data of the trace to be 
modified which can be a useful advanced feature.

  IRegionListener  Notification of IRegions being created, add and deleted.

  IROIListener  Added
 to IRegions this listener notifies of location. Do not do work in the 
drag notification which takes a lot of CPU or your drag will be very 
slow!
...
The
 ILoaderService is backed by LoaderFactory and either the ILoaderService
...
Your
 new file loader class must extend AbstractFileLoader 
(uk.ac.diamond.scisoft.analysis.io) and optionally (usually) 
implement IMetaLoader and IDataSetLoader. If the file format can load 
information about the data faster than parsing all the data, it is a 
very good idea to implement IMetaLoader. @see org.dawb.gda.extensions.loaders.H5Loader

#### []()Thumbnail Service

The
 thumbnail service should be used when pulling out large numbers of 
thumbnails from a directory of images. It has been optimized in speed 
and memory for doing this task.  
  

#### []()Persistence Service

The
 persistence service can be used to save and load data, regions, masks 
to and from HDF5 files. The persistence service can be obtained like the
 following:  
  

IPersistenceService service = (IPersistenceService)ServiceManager.getService(IPersistence.class);  
  
Then in order to save data the method 

service.createPersistenceFile(filePath) needs to be called.It returns an 

IPersistentFile that will be used save data using setters. To read data, 

service.getPersistenceFile(filePath

) has to be used with the getters available to the 

IPersistentFile.  
  
Once the reading/saving is done, the 

IPersistentFile needs to be closed:  
Reading ROIs Example:  
  

IPersistentFile file = 

service.createPersistenceFile(filePath);  
... = file.getROIs();  
file.close();  
...
One
 of the more important concepts in Dawn are regions. These replace the 
old overlays in SDA and integrate elegantly with the region of interest 
(or ROI) code from the mathematics plugin of Dawn 
(uk.ac.diamond.scisoft.analysis). The regions follow the same 
methodology for creation programmatically:

- region = 
createRegion(...)    // The plotting system mouse will change to show 
that clicking in the plot will start to //                               
           add the region, or you can continue and add the region 
programmatically
...
Tools
 can be added to Dawn using the extension point 
"org.dawb.common.ui.toolPage". You should declare a class extending 
AbstractToolPage and some auxiliary information such as if it is a 1D or
 2D tool and the icon to use.
...
It
 is possible to contribute actions to tools either directly when 
programming the tool but also through eclipse extension points. The 
extension point for contributing actions to a tool is called 
"org.dawb.common.ui.toolPageAction". The action requires the id of the 
tool and the id of the command that the action should run. Other options
 such as Icon and where to place the tool are possible.  NOTE this 
system is separate to the eclipse system for adding actions by extension
 point.

#### []()Tools and Perspectives

Tools
 are noramlly chosen by the user via the plotting menu to open a tool. 
Users may also choose to keep a tool open while opening other tools by 
opening the tool in a dedicated view. Another option is for the creator 
of a perpective to open the tool by adding it directly to a perspective.
 This is done in the IPerspectiveFactory implementation by opening a 
view with an id of 
"org.dawb.workbench.plotting.views.toolPageView.fixed" and a secondary 
id equal to the tool concerned.
...
workbench.plotting.tools.region.editor");

### []()Slicing

Any
 workbench part containing plotting may also slice data from files with 
multi-dimensional datasets (like nexus/df5 for instance). Here is the 
recipe for doing this:

**1. **Your view or editor with plotting must implement ISlicablePlottingPart. It has one method:

**void** updatePlot(**final**
ITransferableDataObject[] selections, 
...
**final** ILoaderService service = (ILoaderService)PlatformUI._getWorkbench_().getService(ILoaderService.**class**);

IDataHolder 

holder = service.getData(filePath, **new** ProgressMonitorWrapper( monitor));

ILazyDataset 

lz = holder.getLazyDataset("/entry/path/to/my/data");

The  can now be sliced to get parts of the data. Also lz.getSlice() will read all of the data (use with caution).
...
    **final** List&lt;String&gt;
names = hFile.getDatasetNames(IHierarchicalDataFile._NUMER_ARRAY_);

    **if**
(names.size()!=noResults) **throw****new** Exception("Unexpected data sets from test
file '"+afile+"' in hdf5 file '"+h5.getName()+"' they were:  "+names);
...
       file      = HierarchicalDataFactory._getReader_(getFilePath());

       Group grp =
(Group)file.getData(FUNCTION_ENTRY);

       **if** (grp==**null**) **throw****new** Exception("Reading
Exception: " +FUNCTION_ENTRY+ " entry does not exist in the file " + filePath);

 

        List&lt;HObject&gt; children
=  grp.getMemberList();

        **if** (names==**null**) names = **new**
ArrayList&lt;String&gt;(children.size());
...
       **final** Group entry =
file.group("entry"); // Creates a group if needed, also file.group(entry, "child")

       file.setNexusAttribute(entry,
Nexus._ENTRY_);  

       **final** AbstractDataset a =
....

       **final** Datatype        d = H5Utils._getDatatype_(a);

       **final****long**[]      shape = H5Utils.getLong(a.getShape());

       **final** Dataset s =
file.createDataset(a.getName(),  d, shape, a.getBuffer(), entry);

       file.setNexusAttribute(s, Nexus._SDS_);
...
    // Lock for low level in case anyone else
reads:

    **try** {

        HierarchicalDataFactory._acquireLowLevelReadingAccess_(
...
        // Open in the low
level using something like:

        **int** fapl = H5._H5Pcreate_(HDF5Constants._H5P_FILE_ACCESS_);
...
        HierarchicalDataFactory._releaseLowLevelReadingAccess_(
...
1. Where you create your plotting system and after you have attached most of your listeners etc.

   (for instance createPartControl(...) ), put the lines:

**this**.connection
= **new** ScriptingConnection(getPartName()); // Or whatever name the scripting should use
...
The new intro page in DAWN 1.3 is using a org.dawnsci.intro.universal which is a custom [Eclipse universal intro](http://www.eclipse.org/eclipse/platform-ua/proposals/shared-intro/shared-intro.htm#enhancements).
...
Then you need to create the file introData.xml in the intro folder accordingly:

&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;introContent&gt;
   &lt;extensionContent id="org.dawnsci.intro.workbench" style="style.css" name="DAWN Workbench"   
      path="overview/@"&gt;
      &lt;group style-id="content-group" id="workbench-group"&gt;
          &lt;link label="My Contributed Topic" url="http://org.eclipse.ui.intro/showPage?id=myPageID"   
            id="myLinkID"&gt;
              &lt;text&gt;explanation to topic  &lt;/text&gt;
     	  &lt;/link&gt;

     &lt;page id="myPageID" style="/intro/overview.css" style-id="page"&gt;
     &lt;title style-id="intro-header"&gt;What is DAWN&lt;/title&gt;
     &lt;include path="overview/navigation-links"/&gt;
     &lt;group id="page-content"&gt;
           &lt;group id="content-header" label="What is DAWN" filteredFrom="swt"&gt;
           &lt;/group&gt;
...
The attribute path="overview/@" means you contribute to the Overview section in the Welcome page. The allowed sections are:
...
In order to include a cheat sheet on the front page, you need a link like the following:

&lt;link label="Open introductory cheat sheet" url="http://org.eclipse.ui.intro/showStandby?partId=  
org.dawnsci.intro.cheatsheet&amp;input=org.dawb.workbench.ui.editors.imagesbasic"&gt;  
 &lt;text&gt;This cheat sheet is an introductory tutorial&lt;/text&gt;
&lt;/link&gt;

The & must be in as & a m p ; and not &. Note you 
need the org.eclipse.platform plugin for the cheat sheet to display.  
 The command _showPage_ can be replaced with several other actions, see under the link section on [http://help.eclipse.org/stable/topic/org.eclipse.platform.doc.isv/reference/extension-points/org_eclipse_ui_intro_config.html#e.link](http://help.eclipse.org/stable/topic/org.eclipse.platform.doc.isv/reference/extension-points/org_eclipse_ui_intro_config.html#e.link)  

For more examples, see the subfolder intro in the plugin org.dawb.workbench.ui.
...
**final** ScrolledComposite scrollComposite = **new** ScrolledComposite(parent, SWT._H_SCROLL_ | SWT._V_SCROLL_);  
**final** Composite content = **new** Composite(scrollComposite, SWT._NONE_);  
content.setLayout(**new** GridLayout(1, **false**));  
content.setLayoutData(new GridData(SWT._FILL_, SWT._FILL_, **true**, **true**));  
...  
...
**public****void** controlResized(ControlEvent e) {  
		...
		int height = content.computeSize(r.width, SWT._DEFAULT_).y;  
		scrollComposite.setMinHeight(height);  
		scrollComposite.setMinWidth(content.computeSize(SWT._DEFAULT_, r.height).x);  
	}  
});
